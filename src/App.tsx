    ...
    <div className="h-[100dvh] w-screen flex flex-col bg-space-900 overflow-hidden text-white font-sans selection:bg-neon-blue/30">
      ...
      <header className="h-16 border-b border-space-700 bg-space-800/80 backdrop-blur-md flex items-center px-3 md:px-6 justify-between z-50 shrink-0">
        ...
        <div className="flex items-center gap-2 md:gap-4 shrink-0">
          ...
          <div className="flex items-center gap-1 md:gap-2 ml-1 min-w-0">
            <Wallet className="text-neon-gold w-4 h-4 md:w-6 md:h-6 drop-shadow-[0_0_4px_rgba(255,215,0,0.5)] shrink-0" />
            <span className="text-sm md:text-2xl font-orbitron neon-text-gold tabular-nums tracking-wider transition-transform duration-100 hover:scale-[1.02] truncate">
              {formatNumber(credits)} <span className="text-[8px] md:text-xs">CR</span>
            </span>
          </div>
        </div>

        {/* Status Indicators */}
        <div className="flex items-center gap-2 md:gap-8 flex-1 justify-end max-w-[500px] min-w-0">
          {/* Storage Bar - Expanded */}
          <div className={`flex flex-col items-stretch gap-0.5 md:gap-1 flex-1 transition-all duration-300 min-w-0 ${storageFillRatio >= 1 ? 'ring-1 ring-neon-blue rounded-lg px-1 md:px-2 py-0.5 md:py-1 shadow-[0_0_15px_rgba(0,242,255,0.3)] animate-pulse' : ''}`}>
            <div className="flex items-center justify-between text-[7px] md:text-[10px] uppercase font-orbitron text-gray-400">
              <span className="flex items-center gap-1 truncate"><Package size={10} /><span className="hidden xs:inline">{t.ui.storage}</span></span>
              <span className="tabular-nums shrink-0">{formatNumber(currentStorage)}/{formatNumber(storage.capacity)}</span>
            </div>
            <div className="bg-space-700 h-1 md:h-3 rounded-full overflow-hidden border border-space-600">
              <div 
            ...
            <button 
              ...
              className="flex gap-1 md:gap-2 mt-0.5 px-0.5 hover:bg-white/5 rounded cursor-pointer transition-colors overflow-hidden"
            >
              {Object.entries(storage.current).map(([type, amount]) => {
                ...
                return (
                  <div key={type} className="flex items-center gap-0.5 md:gap-1 shrink-0">
                    <div className={`w-1 md:w-1.5 h-1 md:h-1.5 rounded-full ${colors[type as keyof typeof colors]}`} />
                    <span className="text-[6px] md:text-[8px] font-mono text-gray-400">{formatNumber(amount)}</span>
                  </div>
                );
              })}
            </button>
          ...
          <div className="flex flex-col items-center gap-0.5 md:gap-1 min-w-[50px] md:min-w-[100px] shrink-0">
            <div className="text-[7px] md:text-[10px] uppercase font-orbitron text-gray-400 flex items-center gap-0.5 md:gap-1">
              <Rocket size={10} /> <span className="hidden xs:inline">{t.ui.transport}</span>
            </div>
            <div className={`px-1 md:px-1.5 py-0.5 md:py-1 rounded text-[7px] md:text-[10px] font-orbitron border uppercase tracking-wider transition-all
              ${!transport.isActive 
              ...
              {!transport.isActive ? t.ui.status_ready.substring(0, 3) + '.' : (transport.state === 'offscreen_wait' ? t.ui.status_loading.substring(0, 4) + '.' : t.ui.status_en_route.substring(0, 4) + '.')}
            </div>
          </div>
        </div>
      </header>
      ...
      <footer className="h-16 md:h-24 border-t border-space-700 bg-space-800/80 backdrop-blur-md flex items-center px-2 md:px-6 justify-center gap-1 md:gap-6 z-50 shrink-0">
        <button 
          onClick={() => !boostActive && activateMiningBurst()} 
          disabled={boostActive}
          className={`group relative flex-1 max-w-[70px] md:max-w-[100px] flex flex-col items-center justify-center p-1 md:p-3 rounded-xl transition-all duration-200 neon-border h-12 md:h-20 cursor-pointer
        ...
        >
          <Zap size={16} className={`mb-0.5 md:mb-1 ${boostActive ? 'text-neon-blue animate-pulse' : 'text-neon-blue md:group-hover:scale-110'} transition-transform`} />
          <span className="text-[7px] md:text-[10px] font-orbitron uppercase text-gray-400 text-center">
        ...
        <button 
          ...
          className={`relative flex-1 max-w-[70px] md:max-w-[100px] flex flex-col items-center justify-center p-1 md:p-3 rounded-xl transition-all duration-200 h-12 md:h-20 neon-border cursor-pointer
        ...
        >
          <Rocket size={16} className="text-neon-gold mb-0.5 md:mb-1" />
          <span className="text-[7px] md:text-[10px] font-orbitron uppercase text-gray-400 text-center">{t.ui.send}</span>
          {currentStorage < storage.capacity * 0.2 && !transport.isActive && (
            <span className="absolute -top-3 md:-top-6 text-[6px] md:text-[8px] text-gray-500 font-mono whitespace-nowrap">{t.ui.min_20}</span>
          )}
        </button>
          ...
          <div className="relative mb-0.5 md:mb-1">
            ...
            <div className="absolute -top-1 -right-1 flex gap-0.5">
              {Array.from({ length: radar.maxEnergy }).map((_, i) => (
                <div key={i} className={`w-1 md:w-1.5 h-1 md:h-1.5 rounded-full border border-black shadow-sm ${i < radar.energy ? 'bg-neon-blue' : 'bg-gray-600'}`} />
              ))}
            </div>
          </div>
          <span className="text-[7px] md:text-[10px] font-orbitron uppercase text-gray-400 text-center truncate w-full">
        ...
        <button 
          ...
          className={`group/lock relative flex-1 max-w-[70px] md:max-w-[100px] flex flex-col items-center justify-center p-1 md:p-3 rounded-xl transition-all duration-200 neon-border h-12 md:h-20
            ${unlocked ? 'bg-space-800 hover:bg-space-700 active:scale-95 cursor-pointer' : 'bg-space-800 opacity-60 cursor-not-allowed'}`}
          disabled={!unlocked}
        >
          {!unlocked && (
            <div className="absolute -top-0.5 -right-0.5 bg-space-700 rounded p-0.5"><Lock size={8} className="text-gray-400" /></div>
          )}
          <Sliders size={16} className={`${unlocked ? 'text-neon-blue' : 'text-white'} mb-0.5 md:mb-1`} />
          <span className="text-[7px] md:text-[10px] font-orbitron uppercase text-gray-400 text-center">{t.ui.upgrades}</span>
        </button>
      </footer>
      ...
      <div className="absolute bottom-24 md:bottom-28 right-4 md:right-6 text-right pointer-events-none opacity-60">
        ...
        <div className="text-xs md:text-xl font-mono">{(drones.reduce((sum, d) => sum + d.miningRate, 0)).toFixed(1)}/s</div>
      </div>
